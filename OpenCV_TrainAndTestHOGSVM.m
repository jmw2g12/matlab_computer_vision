function [SVMClassifier, TotalTestingSet] = OpenCV_TrainAndTestHOGSVM( PosDir, NegDir )
    ...
    for i=1:size(PosFiles,1)
        ...
        PosHogFeatures(i,:)=OpenCV_extractHOGFeatures(imresize(imcrop(imread(strcat(PosDir,'/',PosFiles(i).name)),rect_crop_array),[NaN sqrt(new_size*(image_y/image_x))]));
    end
    for i=1:size(NegFiles,1)
        fprintf('neg(%d)\n',i);
        % Attaches names to the start of the features so as to identify them
        NegHogFeatures(i,:)=OpenCV_extractHOGFeatures(imresize(imcrop(imread(strcat(NegDir,'/',NegFiles(i).name)),rect_crop_array),[NaN sqrt(new_size*(image_y/image_x))]));
    end
    ...
    TestingLabels=repenmat(1,size(PosTestingSet,1),1);
